...
while ($true) {
    ...
    if ($request -match 'GET\s+/(\S*?)(?:\s|$)') {
        ...
        Write-Host "FullPath: '$fullPath' Exists: $([System.IO.File]::Exists($fullPath))"
        ...
        if ([System.IO.File]::Exists($fullPath)) {
            ...
            $body = "<html><body><h1>404 Not Found: $path</h1></body></html>"
        ...
        $bodyBytes = $encoding.GetBytes($body)
        $header = "HTTP/1.1 400 Bad Request`r`nContent-Type: text/html`r`nConnection: close`r`n`r`n"
        $headerBytes = $encoding.GetBytes($header)
        $stream.Write($headerBytes, 0, $headerBytes.Length)
        $stream.Write($bodyBytes, 0, $bodyBytes.Length)
    }
